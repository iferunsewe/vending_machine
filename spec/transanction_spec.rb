...
RSpec.describe Transaction do
  subject(:transaction) { described_class.new(item: item, customer_purse_total: customer_purse_total) }
  let(:item) { Item.new(name: 'Foo', price: 40, quantity: 5) }
  let(:customer_purse_total) { 40 }
  ...
  describe '#amount_still_required' do
    ...
    context 'when there is less customer cash than the price of the item' do
      ...
      it 'returns the items price minus the customer cash' do
        expect(amount_still_required).to eq item.price - customer_purse_total
      end
    end
  end
  ...
  describe '#change' do
    ...
    context 'when there is more customer cash than the price of the item' do
      let(:customer_purse_total) { 50 }
      it 'returns 0' do
        expect(change).to eq customer_purse_total - item.price
      end
    end
